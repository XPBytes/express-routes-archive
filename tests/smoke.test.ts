...
test('it generates url', (t) => {
  const fakeRequest = { hostname: 'test.xpbytes.com', socket: { port: 80 } } as unknown as Request
  t.equals(root.url('level', fakeRequest).toString(), 'http://test.xpbytes.com/level/two')
  t.end()
})

test('it can generate https urls (via constructor)', (t) => {
  const fakeRequest = { hostname: 'test.xpbytes.com', socket: { port: 443 } } as unknown as Request
  const secure = new RoutesArchive(undefined, undefined, true)
  secure.register('lock', '/locked')
  t.equals(secure.url('lock', fakeRequest).toString(), 'https://test.xpbytes.com/locked')
  t.end()
})
...
test('it can have a hostname (via constructor)', (t) => {
  const fakeRequest = { hostname: 'nope.xpbytes.com', socket: { port: 443 } } as unknown as Request
  const secure = new RoutesArchive(undefined, undefined, undefined, 'https://alternate.xpbytes.com/mount-path/')
  secure.register('lock', '/locked')
  t.equals(secure.url('lock', fakeRequest).toString(), 'https://alternate.xpbytes.com/mount-path/locked')
  t.end()
})
